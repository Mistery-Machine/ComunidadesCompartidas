...
document.querySelectorAll('.filtro-categoria').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        if (categoria === 'todos') {
            ...
            anunciosFiltrados = anuncios.filter(emp =>
...
function mostrarPagina() {
    ...
    anuncios.forEach(emp => emp.style.display = 'none');

    // Mostrar solo los de la página actual
    anunciosFiltrados.slice(inicio, fin).forEach(emp => {
        emp.style.display = 'flex';
    });

    // Actualizar información del paginador
    document.getElementById('inicioRango').textContent = anunciosFiltrados.length > 0 ? inicio + 1 : 0;
    document.getElementById('finRango').textContent = Math.min(fin, anunciosFiltrados.length);
