...
const emprendimiento = (app) => {
  app.post("/crear-emprendimiento", requireAdmin, async (req, res) => {
    ...
    if (!resultado.exito) {
      ...
      res.render("formulario-exito", {
        ...
        message:
          "Tu emprendimiento ha sido registrado correctamente y está en proceso de aprobación.",
        message_secundario:
          "El emprendimiento será visible en la pestaña de emprendimientos una vez sea aprobado.",
      });
    }
  });
};
...
const formularioEmprendimiento = (app) => {
  app.get("/formulario-emprendimiento", requireAdmin, async (req, res) => {
    const resultado =
      await servicioCategorias.obtenerCategoriaEmprendimientos();
